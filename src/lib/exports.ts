...
const row = (player: ScoredPlayer, score?: RoleScore) => [...columns.map((key) => player[key]), score?.roleScore, score?.valueScore, score?.dataScore, score?.metaScore, score?.recommendationScore, score?.metaRecommendationScore, score?.legacyRoleScore, score?.recruitmentScore, score?.confidenceScore, score?.attribute.score, score?.stats.score, score?.hidden.score, score?.position.score, score?.value.score, score?.caps.join("; "), score?.warnings.join("; "), score?.strengths.join("; "), score?.weaknesses.join("; "), score?.valuePositives.join("; "), score?.valueConcerns.join("; "), score?.dataNotes.join("; "), score?.metaNotes.join("; ")];
const headers = [...columns, "roleScore", "valueScore", "dataScore", "metaScore", "recommendationScore", "metaRecommendationScore", "legacyRoleScore", "recruitmentScore", "confidenceScore", "attributeScore", "adjustedStatsScore", "hiddenProfileScore", "positionScore", "valueContextScore", "caps", "warnings", "strengths", "concerns", "valuePositives", "valueConcerns", "dataNotes", "metaNotes"];
