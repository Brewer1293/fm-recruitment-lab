...
function RankTable({ players, scores, compareIds, sort, onOpen, onCompare }: { players: ScoredPlayer[]; scores: Map<string, RoleScore>; compareIds: string[]; sort: (key: SortKey) => void; onOpen: (player: ScoredPlayer) => void; onCompare: (id: string) => void }) {
...
function PlayerModal({ player, roleId, slot, onClose }: { player: ScoredPlayer; roleId: RoleId; slot: SlotId; onClose: () => void }) {
  const active = scoreForSlot(player, roleId, slot); return <div className="backdrop" onClick={onClose}><aside className="modal" onClick={(e) => e.stopPropagation()}><button className="modal-close" onClick={onClose}>×</button><span className="eyebrow">Recruitment profile</span><h2>{player.name}</h2><p>{player.club} · {player.nationality} · {player.position}</p><div className="profile-grid"><div><span>Age</span><strong>{player.age ?? "-"}</strong></div><div><span>FM Value</span><strong>{money(player)}</strong></div><div><span>Wage</span><strong>£{fmt(player.wageK)}k/w</strong></div><div><span>Feet</span><strong>{player.preferredFoot ?? `${player.leftFoot ?? "-"} / ${player.rightFoot ?? "-"}`}</strong></div></div><h3>{slot} · {ROLE_CONFIG[roleId].shortName} suitability</h3><div className="big-score"><strong className={scoreClass(active.roleScore)}>{fmt(active.roleScore)}</strong><span>Role Score</span></div><Breakdown score={active} /><h3>All roles</h3>{Object.values(ROLE_CONFIG).map((role) => <div className="role-bar" key={role.id}><span>{role.shortName}</span><i><b style={{ width: `${player.scores[role.id].roleScore}%` }} /></i><strong>{fmt(player.scores[role.id].roleScore)}</strong></div>)}<h3>Why this player scores well</h3><p>{active.strengths.join(", ") || "No standout exported attributes available."}</p><h3>Concerns and caps</h3><p>{[...active.caps, ...active.weaknesses, ...active.warnings].join(", ") || "No major exported-data concerns."}</p><h3>Scoring explanation</h3><p>{active.explanation.join(" ")}</p></aside></div>;
}
